...
__global__ void ransac_kernel(
  ...
  float *current_inliers = NULL;
  int current_inlier_count = 0;

  for (int iter = 0; iter < iterations; iter++) {
    i = curand(&rand_state) % (N - sample_size + 1);
    j = curand(&rand_state) % (N - sample_size + 1);
    k = curand(&rand_state) % (N - sample_size + 1);
    ...
    if (current_inlier_count > *inlier_count) {
      *inlier_count = current_inlier_count;
      ...
      if (current_inliers != NULL) {
        cudaFree(current_inliers);
      }

      cudaMallocManaged(&current_inliers,
                        current_inlier_count * sizeof(float) * 3);
      ...
      for (int n = 0; n < N; n++) {
        float dist = fabs(a * data[n * 3] + b * data[n * 3 + 1] +
                     ...
                     norm_of_plane_normal;
        if (dist <= threshold) {
          current_inliers[inlier_idx * 3] = data[n * 3];
          current_inliers[inlier_idx * 3 + 1] = data[n * 3 + 1];
          current_inliers[inlier_idx * 3 + 2] = data[n * 3 + 2];
          inlier_idx++;
        }
      }

      cudaMemcpyAsync(inliers, current_inliers,
                      current_inlier_count * sizeof(float) * 3,
